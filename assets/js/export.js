...
function exportExcel(mode) {
  ...
  const cashRows=DB.filter(e=>e.cash>0).map(e=>({'Date':fmtDate(e.date),'Type':e.type,'Service':e.service,'Client / Supplier / From':entrySource(e),'Staff / Paid To':e.staff,'Cash Amount ($)':e.cash,'Note':e.note}));
  XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(cashRows.length?cashRows:[{Note:'No cash transactions'}]),'Cash Ledger');
  const tfRows=DB.filter(e=>e.tf>0||e.type==='Expense'||e.type==='Withdrawal').map(e=>({'Date':fmtDate(e.date),'Type':e.type,'Payment Type':e.payType,'Service':e.service,'Client / Supplier / From':entrySource(e),'Staff / Paid To':e.staff,'Transfer Amount ($)':e.tf,'Total Amount ($)':e.amount,'Note':e.note}));
